...
Next we'll demonstrate both methods and then show when in Ruboto we *do* need to explicitly create the intent.
...
- Call `start_ruboto_activity` with a block. Here's the template from the [Ruboto Wiki](https://github.com/ruboto/ruboto/wiki/Using-Activities-and-Intents#block-based-activities) page on the subject:
...
Do you see the trick. So first of all we needed the text from the edit box. So we named it and called the `text` method
on it (Java's method is getText so we could call @e.get_text, @e.getText, or @e.text), and then we passed it
on to the send_message method. 
...
You may also want to note that `@e` could also be just `e` since we aren't calling it elsewhere. But it's a good habit to
get an instance variable for your widgets so that they are more widely accessible.
...
2. Note that after you've displayed your message, you can go back and enter a new message via the back arrow. This handled by
the fact that your new activity is a child view of your orignal view. But we'll get more into views later.

3. I said at the beginning that I'd give more info on when to use intents. I think examples are better than talking. So All I'll say now is
you'll mainly want to explicitly define an intent when you are using an external activity like maps, gps, or camera. (See [link](https://github.com/ruboto/ruboto/wiki/Using-Activities-and-Intents#opening-3rd-party-apps-called-in-the-context-of-an-activity))
